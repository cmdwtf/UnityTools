...
{
	internal sealed class EditorIconCache
	{
		private readonly HashSet<EditorIconData> _icons = new();
		private readonly List<EditorIconData> _iconContentListAll = new();
		private readonly List<EditorIconData> _iconContentListSmall = new();
		private readonly List<EditorIconData> _iconContentListMedium = new();
		private readonly List<EditorIconData> _iconContentListLarge = new();

		public int TotalIconCount { get; private set; }
		public int UniqueIconCount { get; private set; }

		public EditorIconCache()
		{
			InitializeIcons();
		...
		/// <summary>
		/// Finds all the <see cref="Texture2D"/>s in the resources that represent an icon and loads their content.
		/// </summary>
		private void InitializeIcons()
		{
			...
			{
				EditorIconData editorIcon = new(texture);

				if (editorIcon.IsValid)
				{
					_icons.Add(editorIcon);
				}
			}

			foreach (string knownIcon in KnownIcons)
			{
				var editorIcon = EditorIconData.FromIconName(knownIcon);
				...
				EditorIconData existing = _icons.FirstOrDefault(i => i.MatchesName(knownIcon));
				if (existing != null)
				{
					existing.AddAlternateName(knownIcon);
					continue;
				}

				// check for matching textures
				existing = _icons.FirstOrDefault(i => i.Image == editorIcon.Image);
				if (existing != null)
				{
					existing.AddSecondaryTextureName(knownIcon);
					continue;
				}
			...
			// count up how many icons we have, and how many names for them.
			UniqueIconCount = _icons.Count;
			TotalIconCount = _icons.Sum(i => i.Count);
		}

		/// <summary>
		/// Initializes the icon lists used to draw the icon grid in this editor window.
		/// </summary>
		private void InitializeIconContentLists()
		{
			_iconContentListSmall.Clear();
			_iconContentListMedium.Clear();
			_iconContentListLarge.Clear();
			_iconContentListAll.Clear();

			IOrderedEnumerable<EditorIconData> ordered = _icons.OrderBy(i => i.SortName);

			foreach (EditorIconData icon in ordered)
			{
				...
				switch (icon.Size)
				{
					...
					case IconSize.Medium:
						_iconContentListMedium.Add(icon);
						break;
					case IconSize.Large:
						_iconContentListLarge.Add(icon);
						break;
					default:
						throw new System.ArgumentOutOfRangeException(nameof(icon.Size));
				}
			}
		}

		/// <summary>
		/// Returns an enumerable of GUIContent representing the icons the user wishes to see.
		/// </summary>
		/// <param name="iconSize">The sizes of icons to return. This is ignored if a valid query was passed.</param>
		/// <param name="query">The query to filter by, optionally null or empty.</param>
		/// <returns>The enumerable icons.</returns>
		public IEnumerable<EditorIconData> Filter(IconSize iconSize, string query)
		{
			// if no query, just return the requested list
			if (!string.IsNullOrWhiteSpace(query))
			{
				return _iconContentListAll
					.Where(x => x.MatchesSearchQuery(query));
			}

			switch (iconSize)
			{
				case IconSize.Small:
					return _iconContentListSmall;
				case IconSize.Medium:
					return _iconContentListMedium;
				case IconSize.Large:
					return _iconContentListLarge;
				default:
    ...
    public static string[] KnownIcons =
    {
